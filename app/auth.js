...
(function () {
  ...
  function injectUI() {
    ...
    if (!document.getElementById('lp-auth-overlay')) {
      ...
      ov.innerHTML =
        ...
        + '<p style="font-size:11.5px;color:var(--dim,#A2909B);margin:14px 0 0;line-height:1.5">📩 O link de acesso chega por e-mail e <strong style="color:var(--rose,#D43A6E)">provavelmente vai cair no spam</strong>. Confira a caixa de spam / lixo eletrônico.</p>'
        + '</div>';
  ...
  async function signInMagic() {
    ...
    if (el) {
      el.style.color = '';
      el.innerHTML =
        '<div style="margin-top:14px;background:rgba(244,114,182,0.1);border:1.5px solid var(--rose,#D43A6E);border-radius:12px;padding:14px 16px;text-align:left">'
        + '<div style="font-size:14px;font-weight:800;color:var(--green,#16a34a);margin-bottom:8px">✅ Link enviado!</div>'
        + '<div style="font-size:13px;color:var(--text,#1A0E15);line-height:1.55">⚠️ <strong>Ele quase sempre cai no SPAM / Lixo eletrônico.</strong> Procure por <strong>Liplop</strong> por lá, abra o link e marque como <strong>"não é spam"</strong>.</div>'
        + '</div>';
    }
  }
